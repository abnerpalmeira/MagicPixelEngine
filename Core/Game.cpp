...
void Game::CreateToolUI() {
    ...
    tools_ui_ = new UI(renderer_,1,{kScreenWidth-foo,0,foo, kScreenHeight},kFontFilePath);
}

void Game::CreateMaterialUI() {
    int foo = (kScreenWidth >> 4) << 2;
    int bar = (kScreenHeight / 9);
    material_ui_ = new UI(renderer_,1,{kScreenWidth-foo,0,foo, kScreenHeight},kFontFilePath);
...
void Game::Render(){
    ...
    if(debug_mode_){
        for(int i=0;i<64;i++){
            simulation_->chunk_[i].Debug(renderer_,viewport_->scale_);
        }
    }
    if(!toggle_){
