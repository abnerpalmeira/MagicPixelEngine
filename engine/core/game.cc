...
void Game::Update()
{
    ...
    if (ui_mode_ && SDL_PointInRect(&InputManager::Instance()->mouse_position_, &ui_->rect_))
    {
        ...
        if (InputManager::Instance()->mouse_states_[SDL_BUTTON_LEFT] == KeyState::DOWN)
            ui_->Click();
    }
    else if (SDL_PointInRect(&InputManager::Instance()->mouse_position_, &kScreenRect))
    {
        ...
        else if (InputManager::Instance()->mouse_states_[SDL_BUTTON_RIGHT] == KeyState::DOWN)
        {
            simulation_->SetCellInsideCircle(foo, draw_radius_, material_, true);
        }
    }
    if (InputManager::Instance()->key_states_.contains(SDLK_TAB) && InputManager::Instance()->key_states_[SDLK_TAB] == KeyState::JUSTDOWN)
    {
        ui_mode_ = !ui_mode_;
    }
    if (InputManager::Instance()->key_states_.contains(SDLK_r) && InputManager::Instance()->key_states_[SDLK_r] == KeyState::JUSTDOWN)
    {
        debug_mode_ = !debug_mode_;
    }
    if (InputManager::Instance()->key_states_.find(SDLK_s) != InputManager::Instance()->key_states_.end() && (InputManager::Instance()->key_states_[SDLK_s] == KeyState::DOWN || InputManager::Instance()->key_states_[SDLK_s] == KeyState::JUSTDOWN))
    {
